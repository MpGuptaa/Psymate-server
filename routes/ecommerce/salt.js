...
const { paginateQuery, sendWhatsAppMessage } = require("../../utils/Helper");
const { Salt } = require("../../schemas/Salt");
...
router.get("/", async (req, res) => {
  try {
    ...
    const totalDocuments = await Salt.countDocuments(query);
...
router.post("/", async (req, res) => {
  try {
    ...
    if (existingSalt) {
      return res.status(400).json({ message: "Salt already exists" });
    }

    // Create a new item
    const item = new Salt(req.body);
    await item.save();
...
router.put("/status/:id/:status", async (req, res) => {
  try {
    ...
    const item = await Salt.findById(itemId);
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    const product = await Salt.findById(productId);
    ...
    res.status(200).json({
      message: "Salt Marked as discontinued",
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
